...
use crate::graph::{Node, NodeStyle};
...
pub trait NodeExt: Node {
    fn inner_size(&self) -> Size {
        let arity = match self.style() {
            NodeStyle::Small => 1,
            NodeStyle::Medium => self.in_arity().max(self.out_arity()).max(1),
            NodeStyle::Large => (self.in_arity() + self.out_arity()).max(1),
        };
    ...
    fn padding(&self) -> Insets {
        match self.style() {
            NodeStyle::Small => Insets::uniform(2.0),
            NodeStyle::Medium => Insets::uniform(4.0),
            NodeStyle::Large => Insets::new(12.0, 8.0 + PORT_HEIGHT + 2.0 * 4.0, 12.0, 8.0),
        }
    }
    ...
    fn port_label_y(&self, idx: u16, side: Side) -> f64 {
        match self.style() {
            ...
            NodeStyle::Medium => {
                let in_arity = self.in_arity();
                let out_arity = self.out_arity();
            ...
            NodeStyle::Large => {
                (PORT_HEIGHT + PORT_GAP)
                    * (f64::from(idx)
                        + f64::from(u16::from(matches!(side, Side::In)) * self.out_arity()))
            },
        }
    }
